...
{
    public class AJNdJourInqModel : PageModel
    {
        private readonly PcbErpContext _context;
        private readonly PaginationService _pagedService;
        private readonly ITableDictionaryService _dictService;

        public AJNdJourInqModel(PcbErpContext context, PaginationService pagedService, ITableDictionaryService dictService)
        {
            _context = context;
        ...
        public string PageTitle => "AJN00009 傳票查詢";
        ...
        /// <summary>Combine radio: 2=總分類帳(default), 1=總分類帳(含子科目), 0=明細分類帳</summary>
        [BindProperty(SupportsGet = true, Name = "Combine")]
        ...
        public async Task<IActionResult> OnGetExportAsync()
        {
            ...
            try
            {
                ...
                var ws = wb.AddWorksheet("AJN00009");
                ...
                var fileName = $"AJN00009_{DateTime.Now:yyyyMMdd}.xlsx";
        ...
        /// <summary>來源追蹤: 查詢 AJNdJourMain 取得來源單據</summary>
        public async Task<IActionResult> OnGetSourceTraceAsync(string paperNum)
        {
            if (string.IsNullOrWhiteSpace(paperNum))
                return new JsonResult(new { ok = false, message = "缺少 PaperNum" });
            ...
            try
            {
                ...
                cmd.CommandText = "select PaperNum=t1.SourNum, PaperId=t1.PaperId from AJNdJourMain t1 with (nolock) where t1.PaperNum=@pnum";
                var p = cmd.CreateParameter();
                p.ParameterName = "@pnum";
                p.Value = paperNum;
                ...
                return new JsonResult(new { ok = false, message = "找不到來源單據" });
            }
            finally
            {
                if (opened) await _context.Database.CloseConnectionAsync();
        ...
        private async Task<List<Dictionary<string, object?>>> ExecInqAsync(DbConnection conn, string paramStr, int spId, CurdSysItem? item, Dictionary<string, object?> defaults)
        {
            ...
            foreach (var p in paramNames)
            {
                ...
                else if (string.Equals(p, "Combine", StringComparison.OrdinalIgnoreCase))
                {
                    // Delphi default: rdoSum checked => Combine=2
                    val = CombineValue ?? 2;
                }
                ...
                if (val == null)
                {
                    ...
                    else if (string.Equals(p, "Combine", StringComparison.OrdinalIgnoreCase)) val = 2;
                }
        ...
        private static bool IsControlQueryKey(string key)
        {
            ...
            return key.Equals("page", StringComparison.OrdinalIgnoreCase)
                ...
                || key.Equals("Combine", StringComparison.OrdinalIgnoreCase);
        }
