...
/// <summary>
/// FOSdReceiveSub 單身頁面的 PageModel，繼承共用樣板 TableDetailModel
/// </summary>
public class FOSdReceiveSubModel : TableDetailModel<FosdReceiveSub>
{
    private const string DynamicItemId = "FOS00015"; // 委外回廠單 ItemId
    ...
    public FOSdReceiveSubModel(IHttpClientFactory httpClientFactory, PcbErpContext context, ITableDictionaryService dictService)
    ...
    // 單身資料表名稱
    public override string TableName => "FOSdReceiveSub";

    // 單頭資料表名稱
    public override string HeaderTableName => "FOSdReceiveMain";

    // 單身 API 相對路徑
    public override string ApiDetailUrl => "/api/FOSdReceiveSub";
    ...
    private async Task LoadHeaderDataAsync(string paperNum)
    {
        try
        {
            ...
            var headerApiUrl = $"{baseUrl}/api/FOSdReceiveMain/{Uri.EscapeDataString(paperNum)}";
            ...
            // 單頭欄位設定
            var headerFieldDict = _dictService.GetFieldDict("FOSdReceiveMain", typeof(FosdReceiveMain));
            ...
            // Lookup 對應
            var headerLookups = _dictService.GetOCXLookups("FOSdReceiveMain");
            if (HeaderData != null)
            {
