...
{
    public class MPH00016Model : PageModel
    {
        private readonly PcbErpContext _context;
        private readonly PaginationService _pagedService;
        private readonly ITableDictionaryService _dictService;

        public MPH00016Model(PcbErpContext context, PaginationService pagedService, ITableDictionaryService dictService)
        {
            _context = context;
        ...
        public string PageTitle => "MPH00016 進貨驗收單查詢";
        ...
        public async Task<IActionResult> OnGetExportAsync()
        {
            ...
            try
            {
                ...
                var ws = wb.AddWorksheet("MPH00016");
                ...
                var fileName = $"MPH00016_{DateTime.Now:yyyyMMdd}.xlsx";
        ...
        public async Task<IActionResult> OnGetOrderSubAsync(string? sourNum, int? sourItem)
        {
            ...
            try
            {
                await using var cmd = conn.CreateCommand();
                cmd.CommandText = @"
select t1.*, PaperId='MPHdOrderMain'
from MPHdOrderSub t1 with (nolock)
join MPHdOrderCharge t2 with (nolock)
  on t2.PaperNum=t1.PaperNum
 and t2.SourItem=t1.Item
where t2.PaperNum=@SourNum
  and t2.Item=@SourItem";
                var p1 = cmd.CreateParameter();
                ...
                var fields = LoadDictSafe("MPHdOrderSubInq")
        ...
        public async Task<IActionResult> OnGetOutSubAsync(string? sourNum, int? sourItem)
        {
            if (string.IsNullOrWhiteSpace(sourNum) || !sourItem.HasValue)
                return new JsonResult(new { items = new List<Dictionary<string, object?>>(), fields = new List<object>() });
            ...
            try
            {
                await using var cmd = conn.CreateCommand();
                cmd.CommandText = @"
select t1.*, PaperId='MINdOutMain'
from MINdOutSub t1 with (nolock)
join MPHdOrderSub t2 with (nolock)
  on t1.SourNum=t2.PaperNum
 and t1.SourItem=t2.Item
join MPHdOrderCharge t3 with (nolock)
  on t3.PaperNum=t2.PaperNum
 and t3.SourItem=t2.Item
where t3.PaperNum=@SourNum
  and t3.Item=@SourItem";
                var p1 = cmd.CreateParameter();
                p1.ParameterName = "@SourNum";
                p1.Value = sourNum!;
                cmd.Parameters.Add(p1);
                ...
                var fields = LoadDictSafe("MPHdOutSubInq")
                    .Where(f => !string.IsNullOrWhiteSpace(f.FieldName) && (f.Visible ?? 1) != 0)
                    .OrderBy(f => f.SerialNum ?? int.MaxValue)
                    .Select(f => new
                    {
        ...
        private async Task<List<Dictionary<string, object?>>> ExecInqAsync(DbConnection conn, string paramStr, int spId, CurdSysItem? item, Dictionary<string, object?> defaults)
        {
            ...
            catch (SqlException ex) when (Request.Query.ContainsKey("debug"))
            {
                var debugParams = string.Join("; ", execParamValues.Select(x =>
                {
                    var t = x.Value?.GetType().Name ?? "null";
                    var s = x.Value as string;
                    var len = s == null ? "" : $"/len={s.Length}";
                    return $"{x.Name}={x.Value ?? "NULL"}<{t}{len}>";
                }));
                throw new InvalidOperationException(
                    $"SQL failed: {ex.Message}\nEXEC: {execText}\nPARAMS: {debugParams}", ex);
            }
            await using (rd)
            while (await rd.ReadAsync())
            {
