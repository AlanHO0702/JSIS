...
where t1.PaperNum=@SourNum and t1.Item=@SourItem";
        ...
        private async Task<List<Dictionary<string, object?>>> ExecInqAsync(DbConnection conn, string paramStr, int spId, CurdSysItem? item, Dictionary<string, object?> defaults)
        {
            ...
            foreach (var p in paramNames)
            {
                ...
                // Empty-search should mean full-query for these flags.
                if (!hasExplicitQueryValue && Request.Query.ContainsKey("search"))
                {
                    if (string.Equals(p, "iPost", StringComparison.OrdinalIgnoreCase)
                        || string.Equals(p, "notcheck", StringComparison.OrdinalIgnoreCase))
                    {
                        val = null;
                    }
                }
        ...
        {
            // Treat explicit search submit (with paging context) as a valid query action.
            if (query.ContainsKey("spId") && query.ContainsKey("page"))
                return true;
