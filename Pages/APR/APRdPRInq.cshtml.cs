...
{
    public class APRdPRInqModel : PageModel
    {
        private readonly PcbErpContext _context;
        private readonly PaginationService _pagedService;
        private readonly ITableDictionaryService _dictService;

        public APRdPRInqModel(PcbErpContext context, PaginationService pagedService, ITableDictionaryService dictService)
        {
            _context = context;
        ...
        /// <summary>ARM00004=應收帳款查詢, APR00020=應付帳款查詢</summary>
        [BindProperty(SupportsGet = true, Name = "itemId")]
        public string ItemIdParam { get; set; } = "APR00020";

        public string ResolvedItemId => string.IsNullOrWhiteSpace(ItemIdParam) ? "APR00020" : ItemIdParam.Trim();
        ...
        // Detail dict fields for 3 sub-grids
        public List<CURdTableField> DetailDictFields { get; set; } = new();
        public List<CURdTableField> CertifDictFields { get; set; } = new();
        public List<CURdTableField> StrikeDictFields { get; set; } = new();
        ...
        public async Task OnGetAsync()
        {
            ...
            try
            {
                var itemId = ResolvedItemId;
                PageTitle = itemId == "ARM00004" ? "ARM00004 應收帳款查詢" : "APR00020 應付帳款查詢";
                ...
                // Load detail dict fields
                DetailDictFields = LoadDictSafe("APRdPRSubInq");
                CertifDictFields = LoadDictSafe("APRdCertifInq");
                StrikeDictFields = LoadDictSafe("APRdStrikeInq");
                ...
                DefaultQueryValues = await LoadDefaultParamRowAsync(conn, SpId, itemId);
        ...
        /// <summary>單據資料 (Delphi qryDetail): APRdPayRecvSub + AJNdAccId + AJNdSubAccId</summary>
        public async Task<IActionResult> OnGetDetailAsync(string paperNum)
        {
            if (string.IsNullOrWhiteSpace(paperNum))
                return new JsonResult(new { ok = false, rows = new List<object>() });
            ...
            try
            {
                ...
                cmd.CommandText = @"
select t1.*, t2.AccIdName, t3.SubAccName,
       PaperId='APRdPayRecvMain'
from APRdPayRecvSub t1 with (nolock),
     AJNdAccId t2 with (nolock),
     AJNdSubAccId t3 with (nolock)
where t1.PaperNum = @PaperNum
  and t1.AccId = t2.AccId
  and t1.AccId = t3.AccId
  and t1.SubAccId = t3.SubAccId";
                var p = cmd.CreateParameter();
                p.ParameterName = "@PaperNum";
                p.Value = paperNum;
        ...
        /// <summary>憑證明細 (Delphi qryOrderSub): APRdCertifSub/Main + CURdPaperInfo</summary>
        public async Task<IActionResult> OnGetCertifAsync(string paperNum)
        {
            if (string.IsNullOrWhiteSpace(paperNum))
                return new JsonResult(new { ok = false, rows = new List<object>() });
            ...
            try
            {
                await using var cmd = conn.CreateCommand();
                cmd.CommandText = @"
select distinct t1.PaperNum, t1.Item, t1.SourNum, t1.SourItem,
       t1.ItemSpec, t1.Quantity, t1.UnitName, t1.UnitPrice,
       t1.AmountOg, t1.Amount, t1.IsD, t1.SourPaperId,
       t2.PaperDate, t2.SourNum as CertifSourNum, t2.InvoiceNum,
       t4.PaperName,
       PaperId='APRdCertifMain'
from APRdCertifSub t1 with (nolock),
     (APRdCertifMain t2 with (nolock)
      Left Join CURdPaperInfo t4 with (nolock) On t2.PaperId = t4.PaperId),
     APRdPayRecvSub t3 with (nolock)
where t1.PaperNum = t2.PaperNum
  and t2.PaperNum = t3.SourNum
  and IsNull(t3.SourNum, '') <> ''
  and t3.PaperNum = @PaperNum";
                var p = cmd.CreateParameter();
                p.ParameterName = "@PaperNum";
                p.Value = paperNum;
                cmd.Parameters.Add(p);
        ...
        /// <summary>沖帳明細 (Delphi qryOutSubInq): APRdStrikeSource + APRdStrikeMain</summary>
        public async Task<IActionResult> OnGetStrikeAsync(string paperNum)
        {
            if (string.IsNullOrWhiteSpace(paperNum))
                return new JsonResult(new { ok = false, rows = new List<object>() });
            ...
            try
            {
                await using var cmd = conn.CreateCommand();
                cmd.CommandText = @"
select t1.*, t2.PaperDate,
       PaperId='APRdStrikeMain'
from APRdStrikeSource t1 with (nolock),
     APRdStrikeMain t2 with (nolock)
where t1.SourNum = @PaperNum
  and t1.PaperNum = t2.PaperNum
  and t2.Finished in (1, 4)";
                var p = cmd.CreateParameter();
                p.ParameterName = "@PaperNum";
                p.Value = paperNum;
                cmd.Parameters.Add(p);
        ...
        /// <summary>憑證來源追蹤 (Delphi qryCertifId + btnCertifTraceClick)</summary>
        public async Task<IActionResult> OnGetCertifTraceAsync(string certifPaperNum)
        {
            if (string.IsNullOrWhiteSpace(certifPaperNum))
                return new JsonResult(new { ok = false, message = "缺少憑證單號" });
            ...
            try
            {
                await using var cmd = conn.CreateCommand();
                cmd.CommandText = @"
select PaperNum = SourNum, PaperId
from APRdCertifMain with (nolock)
where PaperNum = @PaperNum";
                var p = cmd.CreateParameter();
                p.ParameterName = "@PaperNum";
                p.Value = certifPaperNum;
                cmd.Parameters.Add(p);

                await using var rd = await cmd.ExecuteReaderAsync();
                if (await rd.ReadAsync())
                {
                    var sourNum = rd["PaperNum"]?.ToString() ?? "";
                    var paperId = rd["PaperId"]?.ToString() ?? "";
                    return new JsonResult(new { ok = true, paperNum = sourNum, paperId });
                }
                return new JsonResult(new { ok = false, message = "找不到憑證來源單據" });
            }
            finally
            {
                if (opened) await _context.Database.CloseConnectionAsync();
        ...
        private async Task<List<Dictionary<string, object?>>> ReadAllRowsAsync(DbCommand cmd)
        {
