...
using static SPOdMPSOutSubModel;

/// <summary>
/// SPOdMPSOutSub 單身頁面的 PageModel，繼承共用樣板 TableDetailModel
/// </summary>
public class SPOdMPSOutSubModel : TableDetailModel<SPOdMPSOutSub>
{
    #region 單頭/單身資料屬性
    ...
    /// 注入 HttpClient 與欄位服務
    /// </summary>
    public SPOdMPSOutSubModel(IHttpClientFactory httpClientFactory, PcbErpContext context,ITableDictionaryService dictService)
    ...
    // 單身資料表名稱
    public override string TableName => "SPOdMPSOutSub";

    // 單身 API 相對路徑
    public override string ApiDetailUrl => "/api/SPOdMPSOutSub";

    // 單頭資料表名稱
    public override string HeaderTableName => "SPOdMPSOutMain";
    ...
    public async Task OnGetAsync(string PaperNum)
    {
        ...
        HeaderData = await _httpClient.GetFromJsonAsync<Dictionary<string, object>>(
            $"{baseUrl}/api/SPOdMPSOutMain/{PaperNum}"
        );
        ...
        var headerFieldDicts = _dictService.GetFieldDict("SPOdMPSOutMain", typeof(SPOdMPSOutMain));
        ...
        var headerLookupMaps = _dictService.GetOCXLookups("SPOdMPSOutMain");
