...
namespace PcbErpApi.Pages.DynamicTemplate
{
    public class PaperModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(string itemId)
        {
            ...
            var keyFields = BuildKeySet(master.Mdkey, master.LocateKeys);
            ViewData["KeyFields"] = keyFields.ToArray();

            TableFields = FieldDictList
                .Where(x => x.Visible == 1 || keyFields.Contains(x.FieldName ?? "", StringComparer.OrdinalIgnoreCase))
            ...
            if (LookupDisplayMap == null || LookupDisplayMap.Count == 0)
            {
