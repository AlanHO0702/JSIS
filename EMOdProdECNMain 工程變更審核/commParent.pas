...
uses ProdECNPaper, unit_DLL;

function DoComm(iDo:integer):Boolean;
begin
   case iDo of
     0:frmProdECNPaper.prcDoReOpen;
   end;
...
begin
 ...
 RegisterClasses([TfrmProdECNPaper]);

 Application.CreateForm(TfrmProdECNPaper, frmProdECNPaper);

 rParent.Tag:=frmProdECNPaper.btnTempBasDLLDo.Handle;
 ...
 iDelH:=frmProdECNPaper.Height -
 round(unit_DLL.funGetItemFullHeightDel(sItemId,frmProdECNPaper.qryExec) * rRatio);
 //2019.02.01
 //round(168 * rRatio);

 frmProdECNPaper.pgeBwsDtl.Height:=iDelH; //frmProdECNPaper.Height -168;
