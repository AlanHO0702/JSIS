...
(function () {
    ...
    window.showDictModal = async function (modalId = 'fieldDictModal', tableName = window._dictTableName) {
      ...
      if (!el) { console.warn("[fieldDictModal] ?¾ä???modal:", modalId); return; }
  ...
  window.initFieldDictModal = async function (tableName, modalId = 'fieldDictModal') {
    ...
    if (!tbody) { console.warn("[fieldDictModal] ?¾ä???tbody"); return; }
    ...
    if (!alreadyHasRows) {
      try {
        ...
        if (!QUIET) alert("\u6b04\u4f4d\u6e05\u55ae\u8f09\u5165\u5931\u6557");
        console.warn('[fieldDictModal] è¼‰å…¥å¤±æ?:', err);
      }
    }
  ...
  async function syncDbDataTypes(tbody, dictTableName) {
    ...
    try {
      ...
      console.warn('[fieldDictModal] ?Œæ­¥è³‡æ??‹æ?å¤±æ?:', err);
      return { updated: 0 };
    }
  }
  ...
  (function bindGlobalF3Once() {
    ...
    document.addEventListener('keydown', function (e) {
      ...
      if (!modalId) { console.warn("[fieldDictModal] ?¾ä???modal"); return; }
          ...
          keyMapBody.innerHTML = show.map(m => `
              <tr>
                ...
                <td class="text-center">
                  <button type="button" class="btn btn-outline-danger btn-sm fdm-del-keymap" title="ç§»é™¤">ç§»é™¤</button>
                </td>
              </tr>
          `).join('');
      }
