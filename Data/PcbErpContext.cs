...
namespace PcbErpApi.Data
{
    public partial class PcbErpContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            // FMEdIssueMat Ë§áÂ?‰∏ªÈçµ?çÁΩÆ
            modelBuilder.Entity<FmedIssueMat>(entity =>
            {
                entity.HasKey(e => new { e.PaperNum, e.Item });
            });

            // FMEdIssueLayer Ë§áÂ?‰∏ªÈçµ?çÁΩÆ
            ...
            modelBuilder.Entity<FmedVProcNisToStd>(entity =>
        {
            ...
            .HasKey(e => new { e.TableName, e.FieldName }); // ?ôË£°?πÊ?‰Ω†Á?Ê≠??Ë§áÂ?‰∏ªÈçµÊ¨Ñ‰?

            modelBuilder.Entity<CURdOCXTableFieldLK>()
            .HasKey(e => new { e.TableName, e.FieldName, e.KeyFieldName, e.KeySelfName }); // ?ôË£°?πÊ?‰Ω†Á?Ê≠??Ë§áÂ?‰∏ªÈçµÊ¨Ñ‰?
            ...
            // FOSdÂßîÂ§ñÂá∫Âª†/ÂõûÂª†ÂñÆÊìöË®≠ÂÆö
            ...
            {
                e.HasKey(x => new { x.SystemId, x.ParamId });   // ??Ë§áÂ?‰∏ªÈçµ

                // ÔºàÂèØ?∏Ô??•Ë?‰øùÈö™?çÊ?ÂÆöÈï∑Â∫?
                e.Property(x => x.SystemId).HasMaxLength(8).IsRequired();
                e.Property(x => x.ParamId).HasMaxLength(24).IsRequired();

                e.ToTable(tb => tb.HasTrigger("CURdSysParams_tIU")); // ??EF ?ÉÊîπ?®ÊôÆ??UPDATE
            ...
            {
                // Ë≥áÊ?Â∫´Á?ÂØ¶È?Ë°®Â?ÔºöSPODPoKind  ÔºàÁÖß‰Ω?SQL ?ÑÂ?Â≠óÔ?
                ...
                // PoKind int not nullÔºàÈ?Ë®≠Â∞±?Ø‰ª•Ôºå‰?‰∏ÄÂÆöË??çË®≠ÂÆöÔ?
                ...
                entity.Property(e => e.UseId)
                    .HasMaxLength(8)
                    .IsUnicode(false)            // char(8)Ôºå‰???nvarchar
                    .IsFixedLength()
                    .HasDefaultValue("A001");    // Â¶ÇÊ?Ë≥áÊ?Â∫´Ê??êË®≠?ºÂ∞±‰∏ÄËµ∑ÂØ´
            ...
            modelBuilder.Entity<CurdPaperSelected>(entity =>
           {
            ...
            modelBuilder.Entity<FqcdProcInfo>(entity =>
            {
                ...
                // Ignore ?®Êñº lookup ?ÑÊ?‰ΩçÔ?‰∏çÂú®Ë≥áÊ?Ë°®‰∏≠Ôº?
            ...
            // ?ùÂ?ÊØèÂÄ?entity ??decimal Ê¨Ñ‰??ÇÔ??™Â?Ë®≠Â? HasPrecision
